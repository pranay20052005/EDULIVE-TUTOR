...
import { getRazorpayConfig, verifyWebhookSignature } from "../src/lib/server/razorpay.ts";
...
async function run() {
  ...
  console.log(
    `- Key ID: ${config.keyId} (${config.keyId.startsWith("rzp_test_") ? "TEST MODE" : "LIVE"})`,
  );
  ...
  const studentProfileRes = await dbClient.query("SELECT id FROM students WHERE user_id = $1;", [
    studentUser.id,
  ]);
  ...
  check(
    "POST with missing x-razorpay-signature rejected with HTTP 400",
    missingSigRes.status === 400,
  );
  ...
  check(
    "POST with forged x-razorpay-signature rejected with HTTP 400",
  ...
  check(
    "Order created with server-side calculated price (₹2360)",
    order.amountInr === expectedTotalInr,
  );
  check("Order created with valid Razorpay order ID", order.orderId.startsWith("order_"));

  // Check initial DB payment is 'pending'
  const initDbPay = (
    await dbClient.query("SELECT * FROM payments WHERE provider_order_id = $1;", [order.orderId])
  ).rows[0];
  ...
  const rzpPayment = await executeRealRazorpayTestPayment(
    config,
    order.orderId,
    order.amountInPaise,
    true,
  ...
  check(
    "Razorpay payment captured on sandbox gateway",
    rzpPayment.gatewayStatus === "captured",
    `payment_id: ${rzpPayment.paymentId}`,
  );
  ...
  console.log(
    "  -> [Simulating Browser-Close] Client callback was NOT called. Delivering webhook...",
  );
  ...
  const postWebhookPay = (
    await dbClient.query("SELECT * FROM payments WHERE provider_order_id = $1;", [order.orderId])
  ).rows[0];
  check("DB Payment state transitioned to 'paid'", postWebhookPay.status === "paid");
  check(
    "DB Payment records gateway payment ID",
    postWebhookPay.provider_payment_id === rzpPayment.paymentId,
  );
  ...
  check(
    "Enrollment references paid payment record",
    activeEnrollment.payment_id === postWebhookPay.id,
  );

  // Step 6: Verify Notification Delivered
  const notifRes = await dbClient.query(
    "SELECT * FROM notifications WHERE user_id = $1 AND related_entity_id = $2;",
  ...
  check(
    "Duplicate webhook acknowledged as already_paid",
    dupResult.reason === "already_paid" || dupResult.status === "ignored",
  );

  // Verify database record counts remain strictly 1
  const totalPayCount = await dbClient.query(
    "SELECT COUNT(*) FROM payments WHERE provider_order_id = $1;",
    [order.orderId],
  );
  const totalEnrollCount = await dbClient.query(
    "SELECT COUNT(*) FROM enrollments WHERE student_id = $1 AND subject_id = $2;",
    [student.student_id, subject.id],
  );
  const totalNotifCount = await dbClient.query(
    "SELECT COUNT(*) FROM notifications WHERE user_id = $1 AND related_entity_id = $2;",
    [student.id, subject.id],
  );

  check(
    "Database payments count remains exactly 1 (no duplicates)",
    Number(totalPayCount.rows[0].count) === 1,
  );
  check(
    "Database enrollments count remains exactly 1 (no duplicates)",
    Number(totalEnrollCount.rows[0].count) === 1,
  );
  check(
    "Database notifications count remains exactly 1 (no duplicates)",
    Number(totalNotifCount.rows[0].count) === 1,
  );
  ...
  const rzpFailResult = await executeRealRazorpayTestPayment(
    config,
    failOrder.orderId,
    failOrder.amountInPaise,
    false,
  );
  ...
  const failedDbPay = (
    await dbClient.query("SELECT * FROM payments WHERE provider_order_id = $1;", [
      failOrder.orderId,
    ])
  ).rows[0];
  ...
  const failEnrollCheck = await dbClient.query(
    "SELECT * FROM enrollments WHERE student_id = $1 AND subject_id = $2;",
    [student.student_id, failSubject.id],
  );
  ...
  try {
    await dbClient.query("SET LOCAL ROLE authenticated;");
    await dbClient.query(`SET LOCAL "request.jwt.claim.sub" = '${student.id}';`);
    const rlsFailCheck = await dbClient.query(
  ...
  check(
    "Cancelled/abandoned payment remains in 'pending' status",
    cancelDbPay.status === "pending",
  );

  // Verify no enrollment created
  const cancelEnrollCheck = await dbClient.query(
    "SELECT * FROM enrollments WHERE student_id = $1 AND subject_id = $2;",
    [student.student_id, cancelSubject.id],
  );
  ...
  try {
    await dbClient.query("SET LOCAL ROLE authenticated;");
    await dbClient.query(`SET LOCAL "request.jwt.claim.sub" = '${student.id}';`);
    const rlsCancelCheck = await dbClient.query(
      "SELECT * FROM enrollments WHERE student_id = $1 AND subject_id = $2 AND status = 'active';",
      [student.student_id, cancelSubject.id],
    );
  ...
  const tamperedRes = await serverHandler.fetch(
  ...
  const nonExistentRes = await serverHandler.fetch(
    new Request("http://localhost:3000/api/webhooks/razorpay", {
      method: "POST",
  ...
  check(
    "Webhook safely ignores non-existent order (status: ignored)",
    nonExistentJson.result?.status === "ignored" || nonExistentJson.status === "ignored",
  );
  ...
  console.log("================================================================================");
  console.log(
    `E2E GATE VERIFICATION SUMMARY: ${passedChecks} PASSED, ${failedChecks} FAILED (Total: ${totalChecks})`,
  );
  console.log("================================================================================");
